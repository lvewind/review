...
# bin = CDLL("/home/pjreddie/documents/darknet/libdarknet.so", RTLD_GLOBAL)
...
class Yolov4:
    def __init__(self,
                 ...
                 metaPath="./cfg/coco.storage",
                 thresh=0.25, nms=.45, hier_thresh=.5):
        self.thresh = thresh
        ...
        if not os.path.exists(self.metaPath):
            raise ValueError("Invalid storage file path `" + os.path.abspath(self.metaPath) + "`")
    ...
    def detect_cv2(self, custom_image_bgr, showImage=False, debug=False):
...
yolo4 = Yolov4(configPath="./darknet_v4/cfg/yolov4.cfg",
                weightPath="./darknet_v4/yolov4.weights",
                metaPath="./darknet_v4/cfg/coco.storage")

if __name__ == "__main__":
    # print(performDetect())
    import time

    imcv2 = cv2.imread("D:\Project\Python\PlayFish\_hiworker\darknet_v4\data\dog.jpg")
    time_start = time.time()
    detect_result = yolo4.detect_cv2(imcv2, showImage=True)
